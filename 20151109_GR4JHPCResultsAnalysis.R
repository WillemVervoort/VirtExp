...
for (i in seq_along(flow_stns)) {
  ...
  load(paste("HPC/MD_Projectdata/20160728_",names(flow_stns)[i],"GR4JCalibOutput.Rdata",sep=""))
  ...
  for (j in 1:(nrow(Output$Store)-1)) {
    # testing
    #j <- 1
    # make model
    Mod <- update(Mod, x1=Output$Store[j,6],x2=Output$Store[j,5],
                  x3=Output$Store[j,8],x4=Output$Store[j,7],
...
write.csv(OutputTrends,file=paste(Today,"_GR4JHPC_trendsAnalysis.csv",sep=""))

OutputChiew <- do.call(rbind,Chiew.Res)
write.csv(OutputChiew,file=paste(Today,"_GR4JHPC_ChiewAnalysis.csv",sep=""))
