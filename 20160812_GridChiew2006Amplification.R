...
annualR <- aggregate(Gridrain_zoo,format(time(Gridrain_zoo),"%Y"),sum,na.rm=T)
...
# cumulative sums
for (i in 1:ncol(Gridrain_zoo)) {
   x <- Gridrain_zoo[,i]
   miss <- is.na(Gridrain_zoo[,i])
   #x[miss] <- 0
  cs <- cumsum(ifelse(is.na(Gridrain_zoo[,i]-mean_dailyR[i])==T,
                      0, x - mean_dailyR[i]))
  cs[miss] <- NA
  store[[i]] <- data.frame(Date=time(Gridrain_zoo),data=as.numeric(cs),
...
#   }
  Gridrain_zoo[miss,i] <- NA
}
# lgd.txt <- colnames(Gridrain_zoo)
# legend("bottomleft",lgd.txt,col=1:ncol(Gridrain_zoo),lty=1:ncol(Gridrain_zoo))
plot.df <- data.frame(do.call(rbind,store))
ggplot(plot.df,aes(x=Date,y=data)) + geom_line() + facet_wrap(~station)
# Trends in Eliz, Coch, Hell and many others


annualT <- aggregate(maxT_zoo,format(time(Gridrain_zoo),"%Y"),mean,na.rm=T)
...
save(p,file=paste("C:/Users/rver4657/ownCloud/Virtual Experiments/",Today,"_GridRainfallRunoffPlot.RData",sep=""))
...
# publication quality
tiff(paste(Today,"_GridRainfallRunoffPlot.tif",sep=""),width=16*480,height=12*480,
...
write.csv(gam_an_df,
      file=paste("C:/Users/rver4657/ownCloud/Virtual Experiments/",
                Today,"_GridRainfallRunoff.csv",sep=""),row.names=F)
