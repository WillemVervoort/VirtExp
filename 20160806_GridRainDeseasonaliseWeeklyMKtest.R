...
# this version: Also uses gridded rainfall data
...
# # FLOW, RAINFALL AND TEMPERATURE DATA # #####
 flow_stns <- data.frame("COTT"="410730", "RUTH"="219001", "CORA"="215004",
                         ...
                         "DOMB"="607155")
 # Flow and gridded rainfall data
load("ProjectData/20160804_ClimCh_project_MD.Rdata")

require(reshape)
raingrid_zoo <- reshape(GridRainAllDataout[,c(1,3,7)],idvar="Date",
                        timevar = "Station", direction = "wide")
raingrid_zoo <- zoo(raingrid_zoo[,2:14],
                    order.by=as.Date(raingrid_zoo[,1]))
...
real.df <- data.frame(tau = as.numeric(real[,1]),
                      pvalue = as.numeric(real[,2]),
                      catch=rownames(real),type=rep("real",nrow(real)))
sig.set.f <- rbind(sig.set.a,real.df)
# A histogram of taus
hp <- ggplot(sig.set.a, aes(x=tau)) + geom_histogram(binwidth=0.03,colour="white")
