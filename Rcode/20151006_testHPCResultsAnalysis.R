...
for (i in seq_along(flow_stns)) {
  ...
  load(paste("20150928_",names(flow_stns)[i],"CalibOutput.Rdata",sep=""))
  ...
  if (mean(Output$Store[1:10,1])<Output$Store[11,1]) {
    Mod <- update(Mod, x1=mean(Output$Store[1:10,5]),x2=mean(Output$Store[1:10,6]),
                      x3=mean(Output$Store[1:10,7]),x4=mean(Output$Store[1:10,8]),
                      etmult=mean(Output$Store[1:10,9]),
                  return_state=F)
  } else {
    Mod <- update(Mod, x1=mean(Output$Store[11,5]),x2=mean(Output$Store[11,6]),
                  x3=mean(Output$Store[11,7]),x4=mean(Output$Store[11,8]),
                  etmult=mean(Output$Store[11,9]),
                  return_state=F)
  }
