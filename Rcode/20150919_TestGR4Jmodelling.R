...
# read in the data
load("20150909_ClimCh_project_MD.Rdata")

nc <- 2 # number of cores
n <- 2 # number of SCE runs
registerDoMC(cores=nc) 
# this should equal ncpus 
...
Ofit <- function(mod,Store) {
  bestFit <- fitByOptim(mod,objective = ~hmadstat("viney")(Q, X),
                        samples = 2, sampletype = "all.combinations", 
                        initpars = Store[1:2,5:9], multistart = T)   
  s <- summary(bestFit)
...
Calib.fun <- function(flow,Rain,maxT,station, nr,
                      start.t="1970-01-01", 
                      end.t="1979-12-31") {
