...
# This has the new rainfall stations for HELL and NIVE
...
for (i in seq_along(flow_stns)) {
  temp<-read.csv(paste("IDCJAC0009_",closerainfall_stns[,i],
                       "_1800_Data.csv", sep=""))
...
flow_weekly <- apply.weekly(flow_zoo, mean)
rainfall_weekly <- apply.weekly(rain_zoo, mean)
maxT_weekly <- apply.weekly(maxT_zoo, mean)
...
rain_weekly_stack <- data.frame(Date=time(rainfall_weekly), coredata(rainfall_weekly))
...
# load metadata
zz <- file("README_DataDescription.txt","r+")
