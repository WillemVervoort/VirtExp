...
# this version analyses the results from the gridded rainfall
...
for (i in seq_along(flow_stns)) {
  ...
  pred.data <- window(merge(flow_zoo[,i], Gridrain_zoo[,i], maxT_zoo[,i]),
                      start=start.date, end=end.date)
  colnames(pred.data) <- c("Q","P","E")
  
  # load the relevant output
  load(paste("HPC/MD_Projectdata/20160806_",names(flow_stns)[i],"GR4JGridCalibOutput.Rdata",sep=""))
  # extract the model and update with the parameters
  Mod <- Output$mod
  
  modRes[[i]] <- Output$Store
  ...
  for (j in 1:(nrow(Output$Store))) {
    ...
    test <- list()
    for (k in 1:nrow(mu)) {
      temp <- as.data.frame(cbind((1+mu[k,1]/100)*Gridrain_zoo[,i],
    ...
    ann_rain <- rep(aggregate(Gridrain_zoo[,i],list(year=format(time(flow_zoo),"%Y")),sum,na.rm=T),6)
...
write.csv(OutputTrends,file=paste(Today,"_GridGR4JHPC_trendsAnalysis.csv",sep=""),
          row.names=F)

OutputChiew <- do.call(rbind,Chiew.Res)
write.csv(OutputChiew,file=paste(Today,"_GridGR4JHPC_ChiewAnalysis.csv",sep=""),
          row.names=F)

OutputMod <- do.call(rbind,modRes)

p <- ggplot(OutputMod,aes(station,x2)) + 
  geom_boxplot()
#p <- p +  scale_colour_continuous(name="r.squared",low="red", high="blue") 

p <- p + stat_summary(fun.y=mean, geom="point", shape=16, size=5,aes(col=r.squared))
p
