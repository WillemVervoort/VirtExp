...
for (i in seq_along(flow_stns)) {
  ...
  load(paste("HPC/MD_ProjectData/20160805_",names(flow_stns)[i],"CalibOutput.Rdata",sep=""))
...
write.csv(OutputTrends,file=paste(Today,"_GridSimHydHPC_trendsAnalysis.csv",sep=""),
          row.names=F)

OutputChiew <- do.call(rbind,Chiew.Res)
write.csv(OutputChiew,file=paste(Today,"_GridSimHydHPC_ChiewAnalysis.csv",sep=""), 
          row.names=F)
