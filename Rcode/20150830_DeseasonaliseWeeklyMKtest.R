...
# flow_data_70_10 <- window(flow_data, start = start_date,end = end_date)
# 
# 
# ## Rainfall stations
...
for (i in seq_along(flow_stns)) {
  ...
  bad <- is.na(foo) 
  foo[bad] <- mean(foo,na.rm=T)
  maxT_deseas[,colnames(flow_stns[i])]  <- ds(as.numeric(foo),ic="AIC")$z
  # put NA values back
...
hp <- hp + geom_point(data=real.df,aes(x=tau, y=0,colour=pvalue),
                shape=16,size=5) + 
  scale_colour_brewer(palette="Set1") +
  facet_wrap(~ catch,ncol=5)+ ggtitle("Deseasonalised Streamflow") #+

save(hp,file=paste("C:/Users/rver4657/ownCloud/Virtual Experiments/",
                   Today,"_MKdeseasonalisedStreamflow.Rdata",sep=""))
load("C:/Users/rver4657/ownCloud/Virtual Experiments/20151003_MKdeseasonalisedStreamflow.Rdata")
# draft quality
