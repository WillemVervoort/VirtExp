...
This document follows **Table 2** in the series of models, so the headings (and model numbers) relate to this table. The only exception is model 6, which is dealt with in document 3B. 
...
#   i <- 1
  ...
  gam_TrendOnly <- gls(log10(Flow)~trend, correlation= corCAR1(),
...
Store_Rain <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data$trend <- 1:nrow(gamm.data)
  gam_TrendR <- gls(log10(Rain)~trend, correlation= corCAR1(),
       data=na.omit(gamm.data))
...
Store_GridRain <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data$trend <- 1:nrow(gamm.data)
  gam_TrendGridR <- gls(log10(gridRain)~trend, correlation= corCAR1(),
       data=na.omit(gamm.data))
...
Store_FwR <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data$Flow[gamm.data$Flow==0] <- NA
  gamm.data$trend <- 1:nrow(gamm.data)
  gam_TrendFlow_withR <- gamm(log10(Flow)~s(Rain) + trend, 
...
Store_FwGR <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data$Flow[gamm.data$Flow==0] <- NA
  gamm.data$trend <- 1:nrow(gamm.data)
  gam_TrendFlow_withGR <- gamm(log10(Flow)~s(gridRain) + trend, 
                                  correlation= corCAR1(),
                               data=na.omit(gamm.data))
...
Store_FwRE <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data$Flow[gamm.data$Flow==0] <- NA
  gamm.data$trend <- 1:nrow(gamm.data)
  gam_TrendFlow_withRandE <- gamm(log10(Flow)~s(Rain) + s(Rain, MaxT) + 
...
#for (i in 1:length(Stations[,1])) {                  
  ...
  gam_TrendFlow_withGRE <- gamm(log10(Flow) ~ 
                                s(gridRain,k=10) +
                                s(gridRain,MaxT, k=10) + 
                                trend,
                                correlation= corCAR1(),
                                data=na.omit(gamm.data),
...
Store_FwRE2 <- foreach(i = 1:length(Stations[,1]),
  ...
  gamm.data <- subset(flow_rain_maxT_weekly,
                      flow_rain_maxT_weekly$Station == Stations[i,1])
  gamm.data$Flow[gamm.data$Flow==0] <- NA
  gam_Flow_withRandE <- gamm(log10(Flow)~s(Rain) + s(Rain, MaxT) , 
                                  correlation= corCAR1(),
                             data=na.omit(gamm.data),
                                  control=list(niterEM=5))
...
print(hp)
save(hp, file="../projectdata/Figure8ResidGAM_MDPaper.Rdata")
...
See the comments with model 4b with regard to setting the parameter k. Kept this consistent between model 4b and 5b.
...
Store_FwGRE2 <- foreach(i = 1:length(Stations[,1]),
  ...
  gam_Flow_withGRE <- gamm(log10(Flow) ~ 
                             s(gridRain, k=10) +
                             s(gridRain,MaxT, k=10),
                             correlation= corCAR1(),
                             data=na.omit(gamm.data),
