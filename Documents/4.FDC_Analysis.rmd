...
for (i in 1:4) {
  temp <- subset(weekGridRainAllDataout,
                        as.character(weekGridRainAllDataout$Decade)==  
                        as.character(decades[i]))
  FDCs[[i]] <- tapply(temp[,"gridRain"],
                      list(Station=temp$Station),FDC_gen_w)
}
...
# dev.off()
```
Again, this indicates variation between the decades, but no consistent trend between the decades or for groups of stations. In general this indicates that thre could be changes in the  distribution of rainfall, but that this change is currently not strong enough to be consistently reflected in changes in the frequency curve or the double mass curve.
