...
This script also generates final versions of *Figure 3*, *Figure 9* and *Figure 4* in the manuscript. 
...
# also read in the model performance from part 6
Modelperf <- read.csv("../ProjectData/GR4JHPC_modelperformance.csv")
...
# Create the residual data set for the GAM model
    ...
    Results[j,7:8] <- Modelperf[(i-1)+j,2:3]
  }
...
# also read in the model performance from part 6
Modelperf <- read.csv("../ProjectData/GR4JGridHPC_modelperformance.csv")
...
# also read in the model performance from part 6
Modelperf <- read.csv("../ProjectData/SimHydHPC_modelperformance.csv")
...
# also read in the model performance from part 6
Modelperf <- read.csv("../ProjectData/SimHydGridHPC_modelperformance.csv")
...
colnames(OverallResults_GAMM) <- 
  ...
  c("Method", "Model","Station","trend","p_trend","r.squared","rel.bias")
...
p <- ggplot(plot.df, aes(x = Station, y = trend)) +
  ...
  geom_boxplot() +  facet_wrap(~ Model + Method,ncol=1)
p <- p + stat_summary(fun.y=mean, geom="point", shape=16,
                      size=3,aes(col=sig),lwd=2)
...
OutputChiew <- cbind(rbind(OutputChiewSimHyd,OutputChiewSimHydGrid,
                     ...
                     rep(rep(non_par_eta$stn_eta,each=10),4))
...
p <- ggplot(OutputChiew, aes(x = station, y = eta_p)) +
  ...
  stat_summary(fun.y=mean, geom="point", shape=16, size=3,col="black",lwd=2)

p <- p +  geom_point(aes(x=station, y=np_eta_p),
           shape=17,size=3,colour="gray25") + 
