...
# First rearrange the annual data to a dataframe
flow_an_stack <- stack(as.data.frame(annualF))
raingr_an_stack <- stack(as.data.frame(annualGridR))
maxT_an_stack <- stack(as.data.frame(annualT))

RRgr_df <- data.frame(stn =substr(flow_an_stack$ind,1,4),flow=flow_an_stack$values,
                        rain=raingr_an_stack$values,maxT=maxT_an_stack$values,
                        trend = rep(1:nrow(annualF),ncol(annualF))) 
